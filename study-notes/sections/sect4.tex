...
\Cref{sect:conditional-expectation}).
...
follows the \defn{mixture distribution} of \(F_1,F_2,\dotsc\) with respect to
\(F_{I}\).
...
function} \(\widehat{F}_n\) is defined by
...
On \(\supp{F_j}\), we know \(F_j\) is strictly increasing, so by
\labelcref{it:gen-inv-prop} we have \(X_j=F_j^{-1}(F_j(X_j))=F_j^{-1}(U_j)\)
for all \(j=1,\dotsc,d\). (TODO: explain why ``on \(\supp{F_j}\)'' is enough)
Thus, the distribution function \(F\) of \(X\) is
\begin{align*}
...
discontinuity (jump in this case) there is a rational number in between.

Also, by changing the values of \(T_j(X_j)\) at its discontinuities if
necessary (which only leads to a change on a \(\pr\)-null set as
\(\prob{X_j=x}=0\) for all \(x\in\R\) and there are at most countably many
discontinuities), we may further assume that \(T_j\) is right-continuous at its
discontinuities.

Now, by the right-continuity of \(T_j\), we have
...
Since \(T_j\) is strictly increasing
\end{pf}
