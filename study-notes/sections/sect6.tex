...
0\).  Also, since \(\vect{X}_n\netopr\vect{X}\), we have \(\sup_{k\ge
n}\|\vect{X}_k-\vect{X}\|\eqas\|\vect{X}_n-\vect{X}\|\topr 0\). This implies
that \(\sup_{k\ge n}\|\vect{X}_k-\vect{X}\|\topr 0\), and hence
...
measurable.  If \vc{\(X_n\topr X\)} and \(|X_n|\le Y\) a.s.\ for all \(n\in\N\), for
...
x}\le\expv{|X|}/x~\forall x>0\), which is known as the \defn{Markov's inequality}. Also,
taking \(h(x)=x^2\) gives \(\prob{|X|\ge x}\le\expv{X^2}/x^2~\forall x>0\), which is known
as the \defn{Chebyshev's inequality}.
\end{note}
\begin{proposition}
...
\begin{note}
Since \(F\) is increasing, there are at most countably many discontinuities
(jumps), so \(C(F)^{c}\) is countable, and hence a Lebesgue null set.
Therefore, with \(\vect{X}_n\tod\vect{X}\), we have
\(\lim_{n\to\infty}F_n(\vect{x})=F(\vect{x)}\) a.e. (wrt Lebesgue measure
\(\lambda\)).
\end{note}
\item\label{it:conv-dist-lim-dist-unique} \textbf{Uniqueness of limiting
distribution.} The limiting distribution function \(F\) appearing in the
definition of convergence in distribution is indeed unique.
...
we often rely on the \emph{Portmanteau theorem}, which provides a useful
...
exists sufficiently large \(a>0\) such that\\ \(\sup_{i\in
