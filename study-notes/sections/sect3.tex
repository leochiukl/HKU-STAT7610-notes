...
Such function \(X\) is known as a \defn{simple random variable}, which is
...
\(F(X(\omega))\le u\) iff \(F^{-1}(F(X(\omega)))\le F^{-1}(u)\) for all \(\omega\in\Omega\).
Now, note that the support of \(F\) is \(\supp{F}=\{x\in\R:F(x)-F(x-h)>0\text{
for all \(h>0\)}\}\), and \(F\) is strictly increasing on \(\supp{F}\), because
for all \(x,y\in\supp{F}\) with \(x<y\), by setting \(h=y-x\), we know
\(F(y)-F(y-h)=F(y)-F(x)>0\).
