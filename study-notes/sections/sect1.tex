...
\href{https://math.stackexchange.com/a/603499}{this link}), we can deduce that
...
any \(i\in I\) and any \(I\subseteq \N\)}\}\).
\end{lemma}
\begin{pf}
...
\item For the special case of \(\sigma\)-algebra generated by union of
...
\(\defn{\text{\(\mathcal{B}(\Omega)\)}}:=\sigma(\mathcal{T})=\sigma(\{O:\text{\(O\) open in \(\Omega\)}\}\).
Every set in \(\mathcal{B}(\Omega)\) is called a \defn{Borel set}.
...
open in \(\R\)}\}\subseteq\sigma(\{(a,b):a<b\})\), thus
