...
If \(h\in L_{+}\) \emph{(Tonelli)} or \(h\in L^{1}\) \emph{(Fubini)}, then
\(\omega_1\mapsto\vc{\int_{\Omega_2}^{}h(\omega_1,\omega_2)\odif{\mu_2(\omega_2)}}\)
and \(\omega_2\mapsto\orc{\int_{\Omega_1}^{}h(\omega_1,\omega_2)\odif{\mu_1(\omega_1)}}\)
are \(\mathcal{F}_1\)-measurable and \(\mathcal{F}_2\)-measurable respectively, and
\[
...
L_{+}\), and \emph{Fubini's theorem} refers to the result with \(h\in L_1\).
...
\Cref{subsect:multivar-prob-quant}, we introduce a characterization of
...
positive semi-definite \vc{with every diagonal entry being \(1\)}. This is
...
\begin{note}
In such case, we say that \(\nu_a\) has \defn{density} \(f\) with respect to
...
\item Fix any \(g\in L_{+}\). By \Cref{lma:apx-seq}, there exists a sequence
...
\(F(\vect{x})=F((\vect{-\infty},\vect{x}]=\int_{(\vect{-\infty},\vect{x}]}^{}f\odif{\lambda}
