...
respectively and also make some other slight adjustments, depending on the
...
\end{note}
\item \textbf{Preliminary results.} We first discuss some preliminary results
about the Lebesgue integral of this kind.
\begin{proposition}[Approximating sequence]
\label{prp:apx-seq}
...
sequences also.
\item \emph{(Implications)} Based on this result, we know that the \(X_n\)'s are
included in the supremum ``\(\sup_{\substack{0\le Y\le X,\\ \text{\(Y\)
simple}}} \)'' (\(X_n\nearrow X\) implies \(X_n\le X\) for all \(n\in\N\)).
Indeed, by the \emph{monotone convergence theorem} (\Cref{thm:mct}), the
supremum \( \sup_{\substack{0\le Y\le X,\\ \text{\(Y\) simple}}}
\int_{\Omega}^{}Y\odif{\mu}\) can be obtained as the \emph{limit} of Lebesgue
integrals of these simple \(X_n\)'s as \(n\to\infty\).
\end{remark}
\begin{pf}
``\(\Leftarrow\)'': Since \(X_n\) is simple and hence measurable for every
\(n\in\N\), we know by \labelcref{it:seq-rvs-meas}
...
sum)} \(\sum_{n=1}^{\infty}X_n\in L_{+}\) and
...
\(a=b=1\). By \Cref{prp:apx-seq}, there exist sequences \(\{X_{n}\}\) and
...
here we only consider functions with codomain \vc{\(\R\)} rather than \(\bar{\R}\)
...
\(\mu(|X|>x)=0\), \(x\) is an ``essential'' upper bound for \(|X|\) in the sense
that we have \(|X|\le x\) a.e. The essential supremum is then the least
``essential'' upper bound for \(|X|\), which has the property that
\(|X|\le\esssup{|X|}\) a.e., and \(\esssup{|X|}\le x\) for all ``essential''
upper bounds \(x\) for \(|X|\) (meaning that the ``\(\inf\)'' indeed coincides
with ``\(\min\)'').
...
\item Fix any \(h\in L_{+}\). By \Cref{prp:apx-seq}, there exists a sequence
...
\(\Sigma\in\R^{d\times d}\) is invertible iff it is positive \emph{definite}
(not positive \emph{semi-definite}).  Hence, some covariance matrices can be
non-invertible and we need positive definiteness to ensure the invertibility.
\end{remark}
\end{enumerate}
...
we have \(f=g\) \(\mu\)-a.e.
\item \emph{(Integrability of \(f\))} In general, \(f\) may not be
(\(\mu\)-)integrable. However, if \(\nu_{a}\) is finite, i.e., \(\nu_{a}(\Omega)
=\int_{\Omega}^{}f\odif{\mu}<\infty\), then the nonnegative function \(f\) is
...
\(\nu_a(A)=\int_{A}^{}\vc{\odif{\nu_a}}=\int_{A}^{}\vc{f\odif{\mu}}\)).  Such
...
\(\nu_{n,a}(A_n^c)=\int_{A_n^c}^{}f_n\odif{\mu_n}=0\) by \labelcref{it:quasi-int-prop}, and hence
...
sums of \(f_n\)'s are measurable), by \labelcref{it:seq-rvs-meas} we know that
...
unique and measurable \(f:\Omega\to[0,\infty)\) such that
