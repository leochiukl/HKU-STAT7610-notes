...
A \defn{measure} \(\mu\) on \((\Omega,\mathcal{F})\) is a function such that:
...
\defn{\(\sigma\)-finite}. If we have \(\mu(\Omega)<\infty\), then \(\mu\) is called
...
\end{pf}
\item \label{it:lebesgue-stieltjes-meas}\textbf{Lebesgue-Stieltjes measure.} As mentioned in
...
only do that in \labelcref{it:lebesgue-set-not-borel-set}.
...
\begin{pf}
\begin{enumerate}
\item It follows from \labelcref{it:f-vol-prod-diff}.
\item Omitted.
\item Since \(\R^d=\prod_{j=1}^{d}\R\) is a product space, it can be equipped with
...
\end{enumerate}
\end{pf}
\item\label{it:comp-incr-not-imply-d-incr} \textbf{Componentwise increasing does not imply \(d\)-increasing.} The
...
\(\omega\in\Omega\) is called a \defn{sample point}. If \(\Omega\) is a
countable (finite resp.)\ set, then \((\Omega,\mathcal{F},\pr)\) is said to be
\defn{discrete} (\defn{finite} resp.).  Every \(A\in\mathcal{F}\) is called an
\defn{event}.
...
measure, all properties in \labelcref{it:meas-prop} apply.
\end{enumerate}
...
\end{enumerate}
\item\label{it:dist-fn} \textbf{Characterization of probability measures on \(\R^d\).} In the
...
Also, we have \vc{\(\lambda_F=\pr\)} on \(\mathcal{B}(\R^d)\) for this \(F\).
\item \emph{\(F\) induces \(\pr\):} If \(F\) is a distribution function, then
\vc{\(\pr:=\lambda_{F}\)} (with domain restricted to \(\mathcal{B}(\R^d)\)) is
...
\item For every probability measure \(\pr\) on \(\mathcal{B}(\R^d)\) with
...
\item While the domain of a \(d\)-dimensional distribution function \(F\) is
always \(\R^d\) by definition, sometimes a function \(F\) that satisfies all
the three properties mentioned but with domain being a subset of \(\R^d\) is
also called a distribution function; we shall adopt this convention throughout.
To adapt with the definition above, we can view such function \(F\) as
implicitly extended to a function \(\widetilde{F}\) with domain \(\R^d\), given
by
\[
\widetilde{F}(\vect{x})
=F(\min\{\max\{x_1,a_1\},b_1\},\dotsc,\min\{\max\{x_d,a_d\},b_d\}),
\quad\text{for all \(\vect{x}\in\R^d\),}
\]
where \((a_1,\dotsc,a_d):=\inf\supp{F}\) and
\((b_1,\dotsc,b_d):=\sup\supp{F}\). In words, the extension is done by setting
\(1\) in the ``upper-right'' region and \(0\) in all other regions outside
\(\supp{F}\).
\end{remark}
...
\(\lambda_{F}(\vect{a},\vect{b}]=\Delta_{(\vect{a},\vect{b}]}F\) for all
\(\vect{a}\le\vect{b}\). We now show that \(\lambda_{F}\) (with domain
restricted to \(\mathcal{B}(\R^d)\)) is a probability measure on
\(\mathcal{B}(\R^d)\).
\begin{enumerate}[label={(\arabic*)}]
...
\end{pf}
\item\label{it:cts-df-not-abs-cts} \textbf{An example of continuous
distribution function that is not absolutely continuous.} While we know from
\labelcref{it:abs-cts-df-cts} that absolutely continuous distribution functions
are always continuous, the converse does not hold. A standard counterexample is
the \emph{Cantor distribution function}, a.k.a.\ the \emph{devil's staircase}.
(See \url{https://en.wikipedia.org/wiki/Cantor_function} for a picture
of its graph, which indeed looks like a staircase.)
